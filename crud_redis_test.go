...
func TestVirtualFile_Remove_Redis(t *testing.T) {
	...
	fse, err := CreateFileSystem(fsID, homePath, fileSizeTest, blockSizeTest, &eventListener, log.GetScope("test"), &redisMock)
	assert.Equal(t, nil, err)
	...
	// can not remove opened virtual files
	for i := 0; i < TestSize; i++ {
	...
	err = fse.Close()
	assert.Equal(t, nil, err)
	fse, err = ParseFileSystem(fsID, homePath, &eventListener, log.GetScope("test"), &redisMock)
	assert.Equal(t, nil, err)
	...
	for i := 0; i < TestSize; i++ {
		vfs[i], err = fse.OpenVirtualFile(testIDs[i])
		assert.Equal(t, nil, err)
	}
...
func TestVirtualFile_Open_Redis(t *testing.T) {
	homePath, err := os.UserHomeDir()
	assert.Equal(t, nil, err)
	_ = utils.DeleteFile(homePath + "/" + constants.FsPath)
	_ = utils.DeleteFile(homePath + "/" + constants.HeaderPath)
...
func TestVirtualFile_RemoveUnsetBlocks_Redis(t *testing.T) {
	homePath, err := os.UserHomeDir()
	assert.Equal(t, nil, err)
	_ = utils.DeleteFile(homePath + "/" + constants.FsPath)
	_ = utils.DeleteFile(homePath + "/" + constants.HeaderPath)
