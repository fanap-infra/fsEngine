...
//func TestIO_TwoTimesConcurrentMultipleVirtualFile(t *testing.T) {
//	homePath, err := os.UserHomeDir()
//	assert.Equal(t, nil, err)
//	_ = utils.DeleteFile(homePath + "/" + constants.FsPath)
//	_ = utils.DeleteFile(homePath + "/" + constants.HeaderPath)
...
//			defer wg.Done()
//			size := 0
//			bytes[j] = append(bytes[j], [][]byte{})
//			for {
//				token := make([]byte, uint32(rand.Intn(MaxByteArraySize)))
//				m, err := rand.Read(token)
//				assert.Equal(t, nil, err)
//				mu.Lock()
//				bytes[j][fileNumber] = append(bytes[j][fileNumber], token)
//				mu.Unlock()
//				size = size + m
//				n, err := vf.Write(token)
//				assert.Equal(t, nil, err)
